...
def _get_intelligent_agent(inventory_contents: list[int] = None, abilities: list[str] = None,
    ...
    entity = CombatEntity(name="Test Intelligent Agent", abilities=abilities, combat_provider="intelligent_agent")
...
@pytest.mark.parametrize("inventory_contents, expected_items")
def test_restorative_items(inventory_contents: list[int], expected_items: list[int]):
    ...
    ri = [item.id for item in entity.choice_provider.restorative_items]
