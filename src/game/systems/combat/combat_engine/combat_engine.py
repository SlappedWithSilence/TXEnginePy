...
from game.systems.combat.combat_engine.termination_handler import \
    TerminationHandler, PlayerResourceCondition, \
...
class CombatEngine(FiniteStateDevice):
    ...
    def __init__(self, ally_entity_ids: list[int], enemy_entity_ids: list[int],
        ...
        if override_primary_resource is not None:
            self._backup_primary_resource = get_config()['resources'][
                'primary_resource']
            get_config()['resources'][
                'primary_resource'] = override_primary_resource
        else:
        ...
        self._allies: list[entities.CombatEntity] = [entity_manager[e_id] for
                                                     e_id in
                                                     ally_entity_ids]  # alies
        self._enemies: list[entities.CombatEntity] = [entity_manager[e_id] for
                                                      e_id in
                                                      enemy_entity_ids]  # enemies
    ...
    def _compute_turn_order(self):
        ...
        if self._turn_order is None or len(self._turn_order) == 0:
            self._turn_order = [
                weakref.proxy(e) for e in self._allies + self._enemies
            ]
    ...
    def _handle_use_item(self, item_id: int) -> None:
        ...
        if not isinstance(item_instance, Usable):
            raise TypeError(
                f"Cannot use {item_instance}! Not an instance of Usable!"
            )
            ...
            TextEvent(
                [
                    ...
                    ability.on_use.format(
                        ...
                        target={", ".join([t.name for t in _targets])}
                    )
                ]
            ))
    ...
    # Public methods
    def get_relative_allies(self, entity: entities.CombatEntity) -> list[
        ...
        if entity in self.allies:
            ...
            raise CombatError(
                f"Failed to get relative allies! Unknown entity: {entity}")

    def get_relative_enemies(self, entity: entities.CombatEntity) -> list[
        entities.CombatEntity]:
        """
    ...
    def get_valid_ability_targets(self, entity: entities.CombatEntity,
        ...
        if not isinstance(entity, entities.CombatEntity):
            raise TypeError(
                f"argument `entity` is not of type CombatEntity! Got "
                f"{type(entity)} instead.")

        if type(ability_name) is not str:
            raise TypeError(
                f"argument `ability_name` is not of type str! Got "
                f"{type(ability_name)} instead.")

        target_mode: TargetMode = from_cache(
            "managers.AbilityManager").get_instance(ability_name).target_mode
    ...
    def submit_entity_choice(self, entity, choice: ChoiceData) -> None:
        ...
        if not isinstance(entity, CombatEntity):
            raise TypeError(
                f"Entity's that submit choices must be of type CombatEntity! "
                f"Got {type(entity)} instead.")

        if entity != self.active_entity:
            raise RuntimeError(
                "An entity that is not the active entity has submitted a choice"
            )

        # Type and value checking
        if choice is not None and type(choice) is not ChoiceData:
            raise TypeError(
                f"Unknown type for entity choice: {type(choice)}. "
                f"Expected type ChoiceData!")
    ...
    def handle_turn_action(self, choice: ChoiceData) -> None:
        ...
        if choice.choice_type == ChoiceData.ChoiceType.ITEM:
            ...
            raise TypeError(
                f"Unexpected type for choice! Expected str, int, None, got "
                f"{type(choice)} instead!")
    ...
    def active_entity(self) -> entities.CombatEntity:
        ...
        return self._turn_order[self.current_turn] if self.current_turn < len(
            self._turn_order) else None
    ...
    def next_entity(self) -> entities.CombatEntity | None:
        ...
        """
        return self.current_turn + 1 if self.current_turn < len(
            self._turn_order) - 1 else None
    ...
    def _get_default_termination_conditions(cls) -> list[TerminationHandler]:
        ...
        return [
            PlayerResourceCondition(
                primary_resource, 0, TerminationHandler.TerminationMode.LOSS,
                GroupResourceCondition.Mode.EQUAL_TO),
            EnemyResourceCondition(
                primary_resource, 0, TerminationHandler.TerminationMode.WIN,
                GroupResourceCondition.Mode.EQUAL_TO),
        ]
    ...
    def get_master_phase_order(cls) -> list[CombatPhase]:
        return [
            CombatPhase.START_PHASE, CombatPhase.PRE_ACTION_PHASE,
    ...
    def _build_states(self) -> None:
        ...
        @FiniteStateDevice.state_logic(self, self.States.START_TURN_CYCLE,
                                       InputType.SILENT)
        def logic(_: any) -> None:
        ...
        @FiniteStateDevice.state_logic(self, self.States.START_ENTITY_TURN,
                                       InputType.SILENT)
        def logic(_: any) -> None:
        ...
        @FiniteStateDevice.state_logic(self, self.States.HANDLE_PHASE,
                                       InputType.SILENT)
        def logic(_: any) -> None:
        ...
        @FiniteStateDevice.state_logic(self, self.States.EXECUTE_ENTITY_CHOICE,
                                       InputType.SILENT)
        def logic(_: any):
            self.handle_turn_action(self.active_entity_choice)
            self.set_state(self.States.DETECT_COMBAT_TERMINATION)

        @FiniteStateDevice.state_logic(self,
                                       self.States.DETECT_COMBAT_TERMINATION,
                                       InputType.SILENT)
        def logic(_: any) -> None:
            loss = False
            win = False
            for termination_condition in self._termination_conditions:
                try:
                    if termination_condition.is_conditions_met():
                        if termination_condition.termination_mode.value == \
                                TerminationHandler.TerminationMode.WIN.value:
                            win = True
                        else:
                            loss = True
                    ...
                    logger.error(
                        "An error occurred while running a TerminationHandler")
        ...
        def logic(_: any) -> None:
            ...
            loot_preview = "\n".join(
                [
                    f"{from_cache('managers.ItemManager').get_instance(i).name} x{q}"
                    for i, q in loot.items()
                ]
            )
        ...
        def content() -> dict:
            return ComponentFactory.get(
                [get_config()['combat']['victory_message']])

        @FiniteStateDevice.state_logic(self, self.States.PLAYER_LOSS,
                                       InputType.ANY)
        def logic(_: any) -> None:
            self.set_state(self.States.TERMINATE)
        ...
        def content() -> dict:
            return ComponentFactory.get(
                [get_config()['combat']['loss_message']])

        @FiniteStateDevice.state_logic(self, self.States.NEXT_PHASE,
                                       InputType.SILENT)
        def logic(_: any):
        ...
        @FiniteStateDevice.state_logic(self, self.States.TERMINATE,
                                       InputType.SILENT, override=True)
        def logic(_: any) -> None:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self._backup_primary_resource is not None:
            get_config()['resources'][
                'primary_resource'] = self._backup_primary_resource
